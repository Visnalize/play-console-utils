...
The extension requests the `storage` permission, used exclusively to save your own configuration (shortcuts, app mappings, and canned reply templates) via `chrome.storage.sync` (see "Extension settings" above). It also declares host permission for `https://play.google.com/console/*`, which is required for the content script (quick-reply, parse-review, canned-reply picker, and review navigation all run from the same content script) to run there, and for a background script to read the URL of tabs on that host so it can show the colored toolbar icon only there (grayscale everywhere else). No URL or tab information for any other site is ever read.
