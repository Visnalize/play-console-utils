...
- 🌍 **PPP pricing** — Open a price editor in Play Console, then hit "Open PPP pricing panel" in the extension popup (the button enables itself only where there are price rows). A side panel opens: type one base price, see it converted for every market at [purchasing power parity](https://data.worldbank.org/indicator/PA.NUS.PPP), and fill them all in one go, with a progress bar and a Stop button. Each row is priced in the currency Play Console actually bills it in — several markets are billed in USD or EUR rather than their own currency, and those go through the market exchange rate as well as PPP. Each row shows what Play Console charges there today next to the proposed price, with the percentage up or down, and a toggle re-expresses the whole list in your base currency so you can compare markets like for like. It drives Play Console's own per-row editor, so nothing is saved until you press Save on the page itself. The conversion tables (World Bank PPP factors and exchange rates for 200+ countries) ship with the extension, so nothing is fetched at runtime. Base country, rounding style (charm `.99`/`.90`, nice numbers, or exact), a **custom factor** slider for sitting above or below raw parity, and whether to overwrite already-filled prices all live in the panel itself. Expanded one of Play Console's "Other countries / regions" sections? Hit the rescan button to pick the new rows up.
...
`pnpm ppp:refresh` runs [`scripts/fetch-ppp.mjs`](scripts/fetch-ppp.mjs), which pulls the World Bank's `PA.NUS.PPP` (PPP conversion factor) and `PA.NUS.FCRF` (official exchange rate) indicators and rewrites [`utils/ppp-data.ts`](utils/ppp-data.ts) — the committed conversion table behind the PPP pricing feature. It's the only place a network call happens; the extension itself never fetches anything. Countries whose newest observation is more than ten years old are dropped (a pre-redenomination factor would price absurdly), as are any missing from the script's `CURRENCY_BY_COUNTRY` map — both are reported on stdout when you run it, as are countries with no published exchange rate and any whose rate had to come from a different year than their PPP figure. The generated file is in `.prettierignore` so reformatting can't churn it.
